...
function initialize(){
  ...
  Δdb = new Firebase('https://inventory-ari.firebaseio.com/');
  Δitems = Δdb.child('items');
  Δperson =Δdb.child('person');
  Δperson.on('value', personChanged);/*1 time when started dump data
...
function updateGrandTotal(item)
 {
  ...
  $costTotal.val('$'+ db.statistics.grandtotal +'.00');
}
...
function save(){
  ...
  Δperson.set(person);//update is a function that updates
}
